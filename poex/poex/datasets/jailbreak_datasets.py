    ...
    def __init__(
        ...
        if isinstance(dataset, list):
            ...
            if os.path.exists(dataset):
                ...
                logging.warning(f'{dataset} is not a local dataset file. Try to load from huggingface repository `Lemhf14/_Datasets`.')
                raw_dataset = load_dataset("Lemhf14/_Datasets", dataset)
            instance_list = [Instance(**sample) for sample in raw_dataset['train']]
            self._dataset = instance_list
        else:
            logging.error('The dataset parameter for Jailbreak_Dataset should be a list of `Instance` or a string indicates the subset you want from Lemhf14/_Datasets')
