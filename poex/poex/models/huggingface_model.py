...
class HuggingfaceModel(WhiteBoxModelBase):
    ...
    def __init__(
        ...
        :param str mode: The mode of the model, 'attack', 'target', 'unaligned'
    ...
    def create_format_str(self):
        if self.mode == 'target' or self.mode == 'attack':
            ...
            self.conversation.messages = []
            with open('./prompts/unaligned_system_prompt.txt', 'r') as f:
                system_message = f.read()
            self.set_system_message(system_message)
            # 从txt导入系统提示
            with open('./prompts/planner_prompt.txt', 'r') as f:
    ...
    def generate(self, messages, input_field_name='input_ids', clear_old_history=True, stop_words: Optional[List[str]] = ["# Query: ","objects = ","assistant"], **kwargs):
        r"""
        ...
        input_ids = inputs['input_ids'].to(self.model.device.index)
        attention_mask = inputs["attention_mask"].to(self.model.device.index)
        input_length = len(input_ids[0])

        # Add stopping criteria for the specified stop words
        stopping_criteria = []
        if stop_words and len(stop_words) > 0:
            from transformers.generation.stopping_criteria import StoppingCriteriaList, StoppingCriteria
            
            class StopWordsCriteria(StoppingCriteria):
                def __init__(self, tokenizer, stop_words, input_length):
                ...
                def __call__(self, input_ids, scores, **kwargs):
                    ...
                    generated_text = self.tokenizer.decode(input_ids[0][self.input_length:], skip_special_tokens=True)
                    
                    # Check if any stop word appears in the generated text
                    for stop_word in self.stop_words:
                        if stop_word in generated_text:
                            return True
                    return False
            
            stopping_criteria = StoppingCriteriaList([StopWordsCriteria(self.tokenizer, stop_words, input_length)])
        
        kwargs.update({"stopping_criteria": stopping_criteria})
        kwargs.update({input_field_name: input_ids})
        kwargs.update({"attention_mask": attention_mask})
...
def from_pretrained(model_name_or_path: str, model_name: str, mode: str , tokenizer_name_or_path: Optional[str] = None, dtype: Optional[torch.dtype] = None, **generation_config: Dict[str, Any]) -> HuggingfaceModel:
    """
    ...
    **Example**
    ...
    return HuggingfaceModel(model, tokenizer, model_name=model_name, mode=mode, generation_config=generation_config)