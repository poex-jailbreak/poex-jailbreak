...
print(f"Loading Llama Guard 3 model on {device}...")
tokenizer = AutoTokenizer.from_pretrained(model_id)
model = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype=dtype, device_map=device)

def moderate(chat):
    """Use Llama Guard 3 to moderate content"""
...
def classify_harmful(moderation_result):
    ...
    # In Llama Guard 3, harmful content is flagged with specific phrases
...
def plot_confusion_matrix(cm, model_name):
    ...
    plt.title(f'Llama-Guard-3-8B - {model_name}',fontsize=16)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.tight_layout()
    
    # Create directory for plots if it doesn't exist
    os.makedirs('confusion_matrices/Llama-Guard-3-8B', exist_ok=True)
    plt.savefig(f'confusion_matrices/Llama-Guard-3-8B/{model_name}_confusion_matrix.png')
    plt.close()
...
def save_results_to_csv(results):
    ...
    csv_file = "Llama-Guard-3-8B_defense_results.csv"
